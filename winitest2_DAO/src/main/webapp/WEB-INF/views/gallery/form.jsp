                ...
                <td>
                    ...
                    <div>
                        ...
                        <small class="text-muted">
                            최대 5개(기존 포함), 개별 10MB, 총 50MB 이하 / 이미지 파일만 가능
                        </small>
                    </div>
                </td>
            </tr>
...
/* 수정 모드: 저장된 내용 에디터에 로드 + 현재 썸네일 라디오 선택 */
(function () {
...
document.getElementById('fileInput').addEventListener('change', function () {
    ...
    fileEntries.filter(function (e) { return !e.deleted; })
               .forEach(function (e) { totalSize += e.file.size; });
...
function removeFile(idx) {
    ...
    entry.imgEl.remove();
    entry.listEl.remove();
    URL.revokeObjectURL(entry.imgEl.src);
...
function validate() {
    ...
    var imgCount = editor.querySelectorAll('img').length;
    if (imgCount === 0) {
        alert('이미지를 최소 1개 등록해야 합니다.');
        return false;
    }
    ...
    /* 에디터 HTML 직렬화: 신규 이미지의 blob URL → __IMG_{idx}__ */
    var clone = editor.cloneNode(true);
    clone.querySelectorAll('img[data-idx]').forEach(function (img) {
        img.src = '__IMG_' + img.dataset.idx + '__';
