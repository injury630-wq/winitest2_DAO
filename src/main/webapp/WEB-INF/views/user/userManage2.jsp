...
<script>
	...
	$(function() {
		...
		// 아이디 실시간 검증 (idRegex 사용)
		$('#userId').on('input', function() {
			var val = this.value.replace(/[^a-zA-Z0-9]/g, '');
			this.value = val; //영문 숫자만 남기기
			if (val === '') {
	...
	function setRegistMode() {
		...
		if (isAdmin) {
			...
			$('#btnReset').prop('disabled', false).removeClass('btn-secondary')
					.addClass('btn-info');
			$('#btnRegist').prop('disabled', false).removeClass('btn-secondary')
					.addClass('btn-primary');
			$('#btnUpdate').prop('disabled', true).removeClass('btn-warning')
					.addClass('btn-secondary');
			$('#btnDelete').prop('disabled', true).removeClass('btn-danger')
					.addClass('btn-secondary');
		} else {
			// 일반 사용자: 초기화·등록·삭제 비활성 (수정만 가능 - 행 선택 후 활성화)
			$('#btnReset').prop('disabled', true).removeClass('btn-info').addClass(
					'btn-secondary');
			$('#btnRegist').prop('disabled', true).removeClass('btn-primary')
					.addClass('btn-secondary');
			$('#btnUpdate').prop('disabled', true).removeClass('btn-warning')
					.addClass('btn-secondary');
			$('#btnDelete').prop('disabled', true).removeClass('btn-danger')
	...
	function setUpdateMode(user) {
		...
		$('#btnRegist').prop('disabled', true).removeClass('btn-primary').addClass(
				'btn-secondary');
		if (isAdmin) {
			// 관리자: 수정·삭제 활성. 단, 본인 삭제 불가
			$('#btnUpdate').prop('disabled', false).removeClass('btn-secondary')
					.addClass('btn-warning');
			$('#btnDelete').prop('disabled', isSelf).removeClass(
					'btn-secondary btn-danger').addClass(
					isSelf ? 'btn-secondary' : 'btn-danger');
		} else {
			// 일반 사용자: 본인만 수정 가능, 삭제 불가
			$('#btnUpdate').prop('disabled', !isSelf).removeClass(
					'btn-secondary btn-warning').addClass(
					isSelf ? 'btn-warning' : 'btn-secondary');
			$('#btnDelete').prop('disabled', true).removeClass('btn-danger')
					.addClass('btn-secondary');
		}
	}
	...
	function updateListRow($row, user) {
		...
		let badgeText = isActive ? '사용가능' : '사용불가';
		$row.find('.badge').removeClass('bg-success bg-danger')
				.addClass(badgeClass).text(badgeText);
	}
