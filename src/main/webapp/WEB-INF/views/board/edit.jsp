...
<div class="container">
    ...
    <form action="board/editProc.do" method="post" enctype="multipart/form-data">
        ...
        <input type="hidden" name="writerPw"      value="<c:out value="${board.writerPw}"/>"/>
        ...
        <table>
            <tr>
                <th>작성자</th>
                <td>
                    <input type="text" class="form-control" value="<c:out value="${board.writerName}"/>" readonly/>
                </td>
                <th>등록일</th>
                <td>
            ...
            <tr>
                ...
                <td colspan="3">
                    <input type="text" name="title" class="form-control" value="<c:out value="${board.title}"/>" maxlength="160"/>
                </td>
            </tr>
            <tr>
                <th><span class="required">*</span>내용</th>
                <td colspan="3">
                    <textarea name="content" class="form-control" rows="8" maxlength="2800"><c:out value="${board.content}"/></textarea>
                </td>
            </tr>
            <tr>
                <th>첨부파일</th>
        ...
        <div class="d-flex justify-content-center gap-2 mt-3">
            <button type="button" class="btn btn-outline-secondary" onclick="goList()">취소</button>
            <button type="submit" class="btn btn-danger" onclick="return validate()">저장</button>
        </div>
    </form>
</div>

<%-- 취소 버튼용 폼: 검색 조건 + 현재 페이지 유지하여 상세로 복귀 --%>
        ...
        if (result.msg === 'success') {
            ...
            result.fileList.forEach(function(file) {
                ...
                row.querySelector('.file-name').textContent =
                    file.orgName + ' [' + file.fileSize.toLocaleString() + ' byte]';
...
document.getElementById('editFileInput').addEventListener('change', function () {
    ...
    let newFileList = document.getElementById('newFileList');
