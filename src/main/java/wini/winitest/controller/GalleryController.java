...
public class GalleryController {
    ...
    /*
     * [GET 허용 이유] list.do 는 POST-only이나 뒤로가기 대비 GET도 허용.
     * detail.do · form.do 는 Ajax 성공 후 goPost()로 진입하므로 POST only.
     */
    ...
    public String galleryDetail(@RequestParam Map<String, Object> param, Model model) throws Exception {
        ...
        if (gallery == null) return "redirect:/gallery/list.do";
    ...
    public Map<String, Object> gallerySave(
            @RequestParam Map<String, Object> param,
            @RequestParam(value = "activeFileNo",  required = false) String[] activeNos,
            @RequestParam(value = "deleteFileNo",  required = false) String[] deleteNos,
            HttpSession session) throws Exception {
        ...
        if (activeNos != null) param.put("activeFileNos", Arrays.asList(activeNos));
        ...
        if (boardNo == null || boardNo.isEmpty()) {
            ...
            Map<String, Object> gallery = galleryService.getGalleryDetail(param);
            if (gallery == null) { result.put("msg", "F"); return result; }
            ...
            if (!isOwner && !isAdmin) { result.put("msg", "X"); return result; }

            param.put("modUser", loginUser.get("userNo"));
            if (deleteNos != null) param.put("deleteFileNos", Arrays.asList(deleteNos));
    ...
    public Map<String, Object> galleryDelete(@RequestParam Map<String, Object> param,
                                              HttpSession session) throws Exception {
        Map<String, Object> result    = new HashMap<>();
        Map<String, Object> loginUser = (Map<String, Object>) session.getAttribute("loginUser");
        Map<String, Object> gallery   = galleryService.getGalleryDetail(param);
